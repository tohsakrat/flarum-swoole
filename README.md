...
3. n+1序列化并行化，合并其中的n+1查询语句（需要co模式）。
4. 性能监控，开启debug后可以看看是什么插件在n+1（需要co模式）。
注意：如果改了设置，不论css、header、footer、插件设置、用户组权限（授予用户不受影响）都需要完整重启一次。
...
性能会随着时间和请求数量逐渐退化，并且只要woker没被杀掉重启，就会一直退化，退化程度（响应时间增加）高达50%-100%。靠配置max_request等方式平滑重启可以解决，但是肯要付出一点性能（比如满载的时候少一个woker，平时一两秒能拉起来，满载就不好说了）
...
因为swoole节省了容器启动的部分时间（不论co模式还是woker模式），小型请求的初始水位降低，用户就能得到几乎及时的反馈（前提是你的机器线路够好）。

对于flarum最后也是最麻烦的性能问题，cpu计算过于密集，能做的有限。


```
觉得flarum本身“很轻”，闲时也能让用户等个一两秒也没关系的别用哈。
