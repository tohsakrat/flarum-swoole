...
5. 增加103 early hint响应头，能够让用户在真正后端返回之前提前加载forum.css，forum.js等资源。但是要透传回客户端需要nginx1.29.8或者nginx之外的网关，否则103会退化成普通的header（需要co模式）。

注意：如果改了设置，不论额外css、header、footer、插件设置、用户组权限（授予用户不受影响）都需要完整重启一次，否则配置不生效，还可能会出现未知问题。
跑了一天多用户反馈不错，压测重载的竞争问题也排查得差不多了，状态污染问题目前没发现，有用得上的佬友自行取用，如果可以求个kudos。脚本只写了一两天，可能还有不完善的地方，另外遇到的性能退化问题虽然有一些办法但始终很好奇为什么，有知道的佬友还望不吝指教。
...
pecl install swoole
```
2. 把非侵入式入口文件`flarum-swoole-woker.php`或者`flarum-swoole-co.php`+`AutoBatchingDataLoader.php`放在flarum根目录（和vendor文件夹平级）
...
但是如果自己照着日志把follow tags、follow users，fof badgegs，fof polls等插件的n+1查询改了，就几乎没有提升了。

而co模式相比woker模式本身有隔离作用域和各种反射的额外开销，纯计算性能还要差一点，所以具体用哪种模式还要根据实际情况。

部分时间woker模式的峰值表现甚至还要略好一些，明明co模式要难写得多，郁闷……但是满负载情况下还是能看出差距，虽然能承受的吞吐量一样，但co模式得益于调度更合理，单个请求返回能快很多。
...
对于flarum最后也是最麻烦的性能问题，cpu计算过于密集，而且因为这东西渲染帖子正文都是要注入权限组，和其他任何插件作者想的数据模型的，目前还没想到啥阉割之外的缓存非游客数据的方法，cpu纯计算阻塞会严重影响主机资源利用率，能做的有限。
