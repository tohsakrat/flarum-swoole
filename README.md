...
pecl install swoole
```
2. 把非侵入式入口文件`flarum-swoole.php`或者`flarum-swoole-co.php`+`AutoBatchingDataLoader.php`放在flarum根目录（和vendor文件夹平级）
...
## 压测
关掉全部缓存机制，完整鉴权和序列化的纯动态请求。主机是虽Platinum 8272CL但gb5单核450分的[netlab洋垃圾](https://browser.geekbench.com/v5/cpu/24274234)（太乐子了必须分享，~~不要买netlab上游~~）主机，除了线路海星~~但是大小包~~一无是处。
### woker模式
