...
# 效果和局限性
提升不算大，相比优化得很好的fpm/lsphp+jit+opcache提升也就100毫秒左右。

co版本对序列化过程中n+1查询问题严重的插件可能能多个数百毫秒的提升，但是如果真的追求性能，自己照着日志把tags、fof folloers，fof badgegs等插件的n+1查询改了，就几乎没有提升了。而co模式相比woker模式本身有隔离状态的额外开销，所以具体用哪种模式还要根据实际情况

对实际用户体验替身较大的场景是，降低每次请求的数据量，比如调低每一页的贴数，拆分原本重型的请求，让用户不用等容器启动的时间，直接拿到刚组装出来的几个帖子内容。

或者让几个小请求并行，这样可以用上cpu的多核性能，单次请求是不论如何都只能在单核跑的。
