...
3.（需要co模式） n+1序列化并行化，合并其中的n+1查询语句
...
而co模式相比woker模式本身有隔离作用域和各种反射的额外开销，空载性能还要差一点，所以具体用哪种模式还要根据实际情况。

部分时间woker模式的峰值表现甚至还要略好一些，明明co模式要难写得多，一顿操作猛如虎，郁闷……只是满负载情况下能承受的吞吐量一样，但单个请求能快很多。
