...
## 总结
co版本对序列化过程中n+1查询问题可能能多个数百毫秒的提升（合并部分可以合并的请求），空闲基本可以跑到峰值的300ms。但是如果自己照着日志把tags、fof folloers，fof badgegs，fof poll等插件的n+1查询改了，就几乎没有提升了。而co模式相比woker模式本身有隔离状态和各种反射的额外开销，所以具体用哪种模式还要根据实际情况。

总的来说即使解决了序列化过程中的n+1问题，视图层计算过重也难以避免，所以有条件还是要上amd的u，
对实际用户体验替身较大的场景是，降低每次请求的数据量，比如调低每一页的贴数，拆分原本重型的请求，让用户不用等容器启动的时间，直接拿到刚组装出来的几个帖子内容。

站点负载不大的情况下，可以拆分请求，让几个小请求并行，这样可以用上cpu的多核性能，单次请求是不论如何都只能在单核跑的。

co版本重写了Serializer依赖的api，并发执行序列化，合并能够合并的查询语句，可以一定程度缓解n+1问题，
