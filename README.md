...
压测中woker模式的表现甚至还要略好一些，明明co模式要难写得多，一顿操作猛如虎，郁闷…

但其实这两者压测表现差不多就说明co模式更好，因为我本来就通过extend优化了大量本来应该被co模式合并的n+1查询，并且woker模式本身cpu缓存命中率高一些，这种大量重复请求的场景woker模式多少带点作弊。
